...
def get_predicted_data(sample_data_token: str,
    ...
    data_path = nusc.get_sample_data_path(sample_data_token) # img path
...
def render_sample_data(
    ...
    if verbose: # you can close it or open it, suggest close
        plt.show()
    plt.close()
...
def parse_args():
    ...
    parser.add_argument('--data_infos_path', type=str, default='./data/nuscenes/tracking_forecasting-mini_infos_val.pkl',
                        help='[data_infos_path] is created by [tools/create_date.py].') # used for v1.0-mini
    # If you use version-"v1.0-trainval", set it to './data/nuscenes/tracking_forecasting_infos_val.pkl''
    parser.add_argument('--result', help='the tracking results json file')
    parser.add_argument(
...
if __name__ == '__main__':
    ...
    # For v1.0-mini, it has 81 tokens, as 81 frames
    sample_token_list = list(results['results'].keys())
    print("num of frames: ", len(sample_token_list)) # 81
    ...
    for i, sample_token in enumerate(sample_token_list):
        ...
        # render
        render_sample_data(sample_token, pred_data=results, out_path=out_path) # get [sample_token]'s info from [results], and save to [out_path]
        pbar.update(1)
    pbar.close()
